...
///  字典转模型管理器
public class SwiftDicToArrModel {

    /// 全局统一访问入口
    private static let sharedManager = SwiftDicToArrModel()
    ...
    func modelInfo(cls: AnyClass) -> [String: String] {
        ...
        let customClass = cls.respondsToSelector("customClassMapping")
            //cls.respondsToSelector(#selector(DictModelProtocol.customClassMapping))
        if customClass {
